...
interface ChatInterfaceProps {
  ...
  showHistory?: boolean; // New prop to control history visibility
}
...
export function ChatInterface({
    ...
    disabled = false,
    className,
    showHistory = true // Default to showing history
}: ChatInterfaceProps) {
  ...
  // Auto-scroll to bottom when new messages arrive or loading state changes, only if history is shown
  useEffect(() => {
    if (showHistory && viewportRef.current) {
  ...
  return (
    // Adjust container: If history is hidden, don't need flex-col or overflow-hidden
    <div className={cn(
        "bg-card",
        showHistory ? "flex flex-col border rounded-lg overflow-hidden shadow-sm" : "", // Original style if history shown
        className
    )}>
      {/* Conditionally render the scroll area and messages */}
      {showHistory && (
      ...
      <div className={cn(
          "p-4 bg-background flex items-center gap-2",
          showHistory ? "border-t flex-shrink-0" : "" // Add border-t only if history is shown
       )}>
        <Textarea
          value={inputMessage}
          onChange={(e) => setInputMessage(e.target.value)}
          onKeyPress={handleKeyPress}
          placeholder={disabled ? "Generate content first..." : "Ask a question..."}
